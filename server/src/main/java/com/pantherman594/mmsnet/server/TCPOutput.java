...
import com.pantherman594.mmsnet.server.localvpn.ByteBufferPool;
import com.pantherman594.mmsnet.server.localvpn.Packet;
import com.pantherman594.mmsnet.server.localvpn.Packet.TCPHeader;
import com.pantherman594.mmsnet.server.localvpn.TCB;
import com.pantherman594.mmsnet.server.localvpn.TCB.TCBStatus;

public class TCPOutput implements Runnable
{
    private static final String TAG = com.pantherman594.mmsnet.server.localvpn.TCPOutput.class.getSimpleName();
    ...
    public TCPOutput(ConcurrentLinkedQueue<Packet> inputQueue, ConcurrentLinkedQueue<ByteBuffer> outputQueue,
                     Selector selector, String clientNumber)
    {
    ...
    public void run()
    {
        ...
        try
        {
            ...
            while (true)
            {
                ...
                Log.i("AAAAAAAA", destinationAddress.getHostName());

                TCPHeader tcpHeader = currentPacket.tcpHeader;
                Log.i("AAAAAAABBBBB", tcpHeader.toString());
    ...
    private void sendImage(ByteBuffer buf) {
        byte[] responseBytes = buf.array();

        Bitmap bmp = BitmapFactory.decodeByteArray(responseBytes, 0, responseBytes.length);

        MMSManager.sendMmsImage(ServerApplication.getInstance().getApplicationContext(), clientNumber, bmp);
    }
}
